...
// ПОКАЗАТЬ ОШИБКУ
function showInputError (popupElement, inputElement, errorMessage, inputErrorClass, errorClass) {
...
// СКРЫТЬ ОШИБКУ
function hideInputError (popupElement, inputElement, inputErrorClass, errorClass) {
  const errorElement = popupElement.querySelector(`.${inputElement.id}-error`);
...
// ВАЛИДАЦИЯ ИНПУТА
export function isValid (popupElement, inputElement, inputErrorClass, errorClass) {
...
function setEventListeners (popupElement, config) {
  const inputList = Array.from(popupElement.querySelectorAll(config.inputElement));
  const popupSubmitButton = popupElement.querySelector(config.submitButtonSelector);
    
  toggleButtonState(inputList, popupSubmitButton, config.inactiveButtonClass);

  inputList.forEach((inputElement) => {
    inputElement.addEventListener('input', () => {
      isValid(popupElement, inputElement, config.inputErrorClass, config.errorClass);
      toggleButtonState(inputList, popupSubmitButton, config.inactiveButtonClass);
    });
  });
}; 
...
export function clearValidation(popupElement, config) {
  ...
  inputList.forEach((inputElement) => {
    hideInputError(popupElement, inputElement, config.inputErrorClass, config.errorClass);
  })
  toggleButtonState(inputList, popupSubmitButton, config.inactiveButtonClass);
}
